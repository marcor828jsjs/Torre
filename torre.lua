...
if hrp then
    ...
    hrp.Position = Vector3.new(-185.83, 769.30, 68.62)
    task.wait(0.1)
    hrp.Position = Vector3.new(183.87, -10.70, 81.34)
    task.wait(0.1)
    hrp.Position = Vector3.new(-11.41, -10.70, 107.27)
    task.wait(0.1)
    hrp.Position = Vector3.new(-20.62, -10.70, 19.46)
    task.wait(0.1)
    hrp.Position = originalPosition
end

-- Configurações iniciais da câmera
local fixedPosition = Vector3.new(60.88, 177.80, 95.27)
...
local monitoramentoCentro = Vector3.new(35.01, 169.13, 54.89)
local monitoramentoRaio = 10.00

-- Configurações da hitbox de fixação
local fixacaoCentro = Vector3.new(49.31, 158.63, 48.62)
local fixacaoRaio = 38.00
...
local function tocarBotao()
    local buttonPart = game.Workspace:FindFirstChild("Gudock")
...
local function gerenciarPlataformaInvisivel()
    if isInFixacaoHitbox() then
        if not plataformaInvisivel then
            ...
            plataformaY = fixacaoCentro.Y -- Altura fixa na coordenada Y da hitbox
        end
...
local function teleportTroll()
    if hrp then
        hrp.CFrame = CFrame.new(19.18, 159.03, 22.44)
    end
end
...
yesBtn.MouseButton1Click:Connect(function()
    ...
    fixedPosition = Vector3.new(60.88, 177.80, 95.27)
    promptGui:Destroy()
end)
